<template>
  <page-container>
    <app-card>
      ...
      <el-form ref="form" :model="listParams" :disabled="listLoading" label-width="70px" @submit.prevent="handleQuery">
        <el-form-item label="商品分类">
          <el-select v-model="listParams.cate_id" placeholder="请选择" clearable @change="loadList">
            <el-option label="全部" :value="0" />
            <el-option v-for="item in productCates" :key="item.id" :label="`${item.html}${item.cate_name}`" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="商品名称">
          <el-input v-model="listParams.store_name" placeholder="请输入商品名称关键字" clearable style="width: 300px">
            <template #append>
              <el-button icon="el-icon-search" @click="loadList" />
            </template>
          </el-input>
        </el-form-item>
        <el-form-item label="商品类目">
          <el-radio-group v-model="listParams.type" @change="loadList">
            ...
            <el-radio v-for="item in productTypes" :key="item.type" :label="item.type">
    ...
    <app-card>
      <template #header>
        <el-button type="primary" icon="el-icon-plus" @click="$router.push('/admin/product/add_product')"> 添加商品 </el-button>
        <el-button :loading="updateProductsShowLoading" @click="handleUpdateProductsShow"> 批量上架 </el-button>
        <el-button :loading="updateProductsUnshowLoading" @click="handleUpdateProductsUnshow"> 批量下架 </el-button>
        <el-button icon="el-icon-document" :loading="exportExcelLoading" @click="handleExportExcel"> 导出表格 </el-button>
      </template>
      <el-table
        :data="list"
        ...
        <el-table-column min-width="120px" label="操作" fixed="right" align="center">
          <template #default="{ row }">
            ...
            <el-popconfirm :title="row.is_del ? '确定恢复商品吗？' : '确定移到回收站吗？'" @confirm="handleDelete(row.id)">
              <template #reference>
                <el-button type="text">
