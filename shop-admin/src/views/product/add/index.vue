      ...
      <el-form ref="form" :model="product" :rules="formRules" label-width="100px">
        ...
        <el-form-item label="商品分类" prop="cate_id">
          <el-select v-model="product.cate_id" multiple style="width: 50%">
            <el-option v-for="item in productCates" :key="item.id" :label="item.cate_name" :value="item.id" :disabled="item.pid === 0" />
          </el-select>
        </el-form-item>
          ...
          <el-space direction="vertical" fill style="width: 100%" alignment="flex-start">
            ...
            <template v-if="multiAttrData.length">
              ...
              <AttrTable v-model="multiAttrData">
                <template #prepend>
                  <el-table-column v-for="item in tableHeader" :key="item.key" :prop="item.key" :label="item.title" />
                </template>
                <template #append>
        ...
        <el-form-item label="活动优先级" prop="activity">
          <el-space>
            ...
            <app-draggable
              ...
              <el-tag v-for="item in activities" :key="item.name" :type="item.type" effect="dark">
                {{ item.name }}
              </el-tag>
            </app-draggable>
          </el-space>
...
const formRules = ref({
  ...
  cate_id: [{ required: true, message: '请选择商品分类', trigger: 'change', type: 'array', min: '1' }],
  ...
  temp_id: [{ required: true, message: '请选择运费模板', trigger: 'change', type: 'number' }],
